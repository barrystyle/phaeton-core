...
class MonitorPlugin extends phaeton_storm_1.BasePlugin {
    static get alias() {
        return 'monitor';
    }
    static get info() {
    ...
    async load(channel) {
        this._app = express();
        const options = phaeton_utils_1.objects.mergeDeep({}, config.defaultConfig.default, this.options);
    ...
    _subscribeToEvents() {
        this._channel.subscribe('app:network:event', (eventData) => {
            ...
            if (event === 'postTransactionsAnnouncement') {
                const errors = phaeton_validator_1.validator.validate(schema_1.transactionAnnouncementSchema, data);
                if (errors.length > 0) {
                    return;
                }
    ...
    _handleFork(block) {
        ...
        const { header } = phaeton_codec_1.codec.decode(this.schemas.block, Buffer.from(block, 'hex'));
        const blockId = phaeton_cryptography_1.hash(header).toString('hex');
        if (this._state.forks.blockHeaders[blockId]) {
            this._state.forks.blockHeaders[blockId].timeReceived = Date.now();
        }
        else {
            const decodedHeader = phaeton_codec_1.codec.decodeJSON(this.schemas.blockHeader, header);
    ...
    _handlePostBlock(data) {
        const decodedBlock = phaeton_codec_1.codec.decode(this.schemas.block, Buffer.from(data.block, 'hex'));
        const decodedBlockHeader = phaeton_codec_1.codec.decode(this.schemas.blockHeader, decodedBlock.header);
        const blockId = phaeton_cryptography_1.hash(decodedBlock.header);
