...
const getTransaction = (channel, codec) => async (req, res) => {
    ...
    if (!transactionId || !phaeton_validator_1.isHexString(transactionId)) {
        res.status(400).send({
...
const postTransaction = (channel, codec) => async (req, res) => {
    const errors = phaeton_validator_1.validator.validate(transactionInputSchema, req.body);
    if (errors.length) {
        res.status(400).send({
            errors: [{ message: new phaeton_validator_1.PhaetonValidationError([...errors]).message }],
        });
        return;
    }
    try {
