...
class ForgerPlugin extends phaeton_storm_1.BasePlugin {
    static get alias() {
        return 'forger';
    }
    static get info() {
    ...
    async _setForgersList() {
        this._forgersList = new phaeton_utils_1.dataStructures.BufferMap();
    ...
    _getForgerHeaderAndPayloadInfo(block) {
        ...
        const forgerAddress = phaeton_cryptography_1.getAddressFromPublicKey(Buffer.from(header.generatorPublicKey, 'hex')).toString('hex');
    ...
    _getForgerReceivedVotes(payload) {
        ...
        for (const trx of payload) {
            if (trx.moduleID === 5 && trx.assetID === 1) {
                const senderAddress = phaeton_cryptography_1.getAddressFromPublicKey(Buffer.from(trx.senderPublicKey, 'hex'));
