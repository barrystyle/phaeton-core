...
const phaeton_transactions_1 = require("@phaeton/phaeton-transactions");
...
class Transaction {
    ...
    async create(input, passphrase, options) {
        ...
        const { publicKey, address } = phaeton_cryptography_1.getAddressAndPublicKeyFromPassphrase(passphrase);
        ...
        if (account.keys && account.keys.numberOfSignatures > 0) {
            return phaeton_transactions_1.signMultiSignatureTransaction(assetSchema, txInput, networkIdentifier, passphrase, account.keys, options === null || options === void 0 ? void 0 : options.includeSenderSignature);
        }
        if ((options === null || options === void 0 ? void 0 : options.multisignatureKeys) && (options === null || options === void 0 ? void 0 : options.includeSenderSignature)) {
            return phaeton_transactions_1.signMultiSignatureTransaction(assetSchema, txInput, networkIdentifier, passphrase, options.multisignatureKeys, options.includeSenderSignature);
        }
        return phaeton_transactions_1.signTransaction(assetSchema, txInput, networkIdentifier, passphrase);
    }
    async get(id) {
    ...
    async sign(transaction, passphrases, options) {
        ...
        const networkIdentifier = Buffer.from(this._nodeInfo.networkIdentifier, 'hex');
        const address = phaeton_cryptography_1.getAddressFromPublicKey(transaction.senderPublicKey);
        const accountHex = await this._channel.invoke('app:getAccount', {
            address: address.toString('hex'),
        });
        const account = codec_1.decodeAccount(Buffer.from(accountHex, 'hex'), this._schema);
        if (account.keys && account.keys.numberOfSignatures > 0) {
            for (const passphrase of passphrases) {
                phaeton_transactions_1.signMultiSignatureTransaction(assetSchema, transaction, networkIdentifier, passphrase, account.keys, options === null || options === void 0 ? void 0 : options.includeSenderSignature);
            }
            return transaction;
        }
        if ((options === null || options === void 0 ? void 0 : options.multisignatureKeys) && (options === null || options === void 0 ? void 0 : options.includeSenderSignature)) {
            for (const passphrase of passphrases) {
                phaeton_transactions_1.signMultiSignatureTransaction(assetSchema, transaction, networkIdentifier, passphrase, options.multisignatureKeys, options.includeSenderSignature);
            }
            return transaction;
        }
        return phaeton_transactions_1.signTransaction(assetSchema, transaction, networkIdentifier, passphrases[0]);
    }
    ...
    computeMinFee(transaction) {
        const assetSchema = codec_1.getTransactionAssetSchema(transaction, this._schema);
        return phaeton_transactions_1.computeMinFee(assetSchema, transaction);
    }
    toJSON(transaction) {
        ...
        const jsonTxAsset = phaeton_codec_1.codec.toJSON(schemaAsset, txAsset);
        const jsonTxRoot = phaeton_codec_1.codec.toJSON(this._schema.transaction, txRoot);
        const jsonTx = {
            ...jsonTxRoot,
            asset: jsonTxAsset,
    ...
    fromJSON(transaction) {
        ...
        delete txRoot.id;
        const schemaAsset = codec_1.getTransactionAssetSchema(txRoot, this._schema);
        const txAssetObject = phaeton_codec_1.codec.fromJSON(schemaAsset, txAsset);
        const txRootObject = phaeton_codec_1.codec.fromJSON(this._schema.transaction, txRoot);
        const txObject = {
            ...txRootObject,
            asset: txAssetObject,
