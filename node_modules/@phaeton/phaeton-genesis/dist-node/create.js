...
const getGenesisBlockSchema = (accountSchema) => phaeton_utils_1.objects.mergeDeep({}, phaeton_chain_1.blockSchema, {
    properties: {
        header: phaeton_utils_1.objects.mergeDeep({}, phaeton_chain_1.blockHeaderSchema, {
            $id: '/block/genesis/header/id',
            properties: {
                id: {
                    dataType: 'bytes',
                },
                asset: phaeton_chain_1.getGenesisBlockHeaderAssetSchema(phaeton_chain_1.getAccountSchemaWithDefault(accountSchema)),
            },
        }),
    },
});
...
const getBlockId = (header, accountSchema) => {
    const genesisBlockAssetBuffer = phaeton_codec_1.codec.encode(phaeton_chain_1.getGenesisBlockHeaderAssetSchema(accountSchema), header.asset);
    const genesisBlockHeaderBuffer = phaeton_codec_1.codec.encode(phaeton_chain_1.blockHeaderSchema, {
        ...header,
        asset: genesisBlockAssetBuffer,
    });
    return phaeton_cryptography_1.hash(genesisBlockHeaderBuffer);
};
const createAccount = (account, defaultAccount) => phaeton_utils_1.objects.mergeDeep({}, phaeton_utils_1.objects.cloneDeep(defaultAccount), account);
const createGenesisBlock = (params) => {
    ...
    const { default: defaultAccount, ...accountSchema } = phaeton_chain_1.getAccountSchemaWithDefault(params.accountAssetSchemas);
...
const getGenesisBlockJSON = (params) => phaeton_codec_1.codec.toJSON(exports.getGenesisBlockSchema(params.accountAssetSchemas), params.genesisBlock);
