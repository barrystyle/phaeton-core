...
const phaeton_tree_1 = require("@phaeton/phaeton-tree");
...
const validateSignature = (publicKey, dataWithoutSignature, signature, networkIdentifier) => {
    ...
    const verified = phaeton_cryptography_1.verifyData(blockWithNetworkIdentifierBytes, signature, publicKey);
    if (!verified) {
...
const getTransactionRoot = (ids) => {
    const tree = new phaeton_tree_1.MerkleTree(ids);
    return tree.root;
};
...
const validateGenesisBlockHeader = (block, accountSchema) => {
    ...
    const errors = [];
    const headerErrors = phaeton_validator_1.validator.validate(phaeton_utils_1.objects.mergeDeep({}, schema_1.blockHeaderSchema, {
        properties: {
            version: {
                const: 0,
            },
    ...
    const assetErrors = phaeton_validator_1.validator.validate(schema_1.getGenesisBlockHeaderAssetSchema(accountSchema), header.asset);
    ...
    if (!phaeton_utils_1.objects.bufferArrayUniqueItems(header.asset.initDelegates)) {
        errors.push({
    ...
    if (!phaeton_utils_1.objects.bufferArrayOrderByLex(header.asset.initDelegates)) {
        errors.push({
    ...
    if (!phaeton_utils_1.objects.bufferArrayEqual(accountAddresses, copiedAddresses)) {
        errors.push({
    ...
    if (!phaeton_utils_1.objects.bufferArrayUniqueItems(accountAddresses)) {
        errors.push({
            dataPath: '.accounts',
            keyword: 'uniqueItems',
            message: 'should NOT have duplicate items',
