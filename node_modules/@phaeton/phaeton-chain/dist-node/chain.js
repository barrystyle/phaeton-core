...
const debug = createDebug('phaeton:chain');
class Chain {
    constructor({ db, genesisBlock, accountSchemas, blockTime, networkIdentifier, maxPayloadLength, rewardDistance, rewardOffset, rewardMilestones, minFeePerByte, baseFees, minBlockHeaderCache = constants_1.DEFAULT_MIN_BLOCK_HEADER_CACHE, maxBlockHeaderCache = constants_1.DEFAULT_MAX_BLOCK_HEADER_CACHE, }) {
        ...
        phaeton_codec_1.codec.addSchema(schema_1.blockSchema);
        phaeton_codec_1.codec.addSchema(schema_1.blockHeaderSchema);
        phaeton_codec_1.codec.addSchema(schema_1.signingBlockHeaderSchema);
        for (const assetSchema of Object.values(this._blockAssetSchema)) {
            phaeton_codec_1.codec.addSchema(assetSchema);
        }
        phaeton_codec_1.codec.addSchema(this._accountSchema);
        phaeton_codec_1.codec.addSchema(schema_1.stateDiffSchema);
    ...
    async genesisBlockExist(genesisBlock) {
        ...
        try {
            ...
            if (!(error instanceof phaeton_db_1.NotFoundError)) {
                throw error;
            }
        }
        let lastBlockHeader;
    ...
    async applyGenesisBlock(block, stateStore) {
        ...
        await stateStore.consensus.set(constants_1.CONSENSUS_STATE_VALIDATORS_KEY, phaeton_codec_1.codec.encode(schema_1.validatorsSchema, { validators: initialValidators }));
    ...
    validateBlockHeader(block) {
        ...
        const errors = phaeton_validator_1.validator.validate(schema_1.blockHeaderSchema, headerWithoutAsset);
        if (errors.length) {
        ...
        const assetErrors = phaeton_validator_1.validator.validate(assetSchema, block.header.asset);
        if (assetErrors.length) {
            throw new phaeton_validator_1.PhaetonValidationError(assetErrors);
        }
    ...
    async getValidators() {
        ...
        const { validators } = phaeton_codec_1.codec.decode(schema_1.validatorsSchema, validatorsBuffer);
        return validators;
    }
    async setValidators(validators, stateStore, blockHeader) {
        ...
        const { validators: previousValidators } = phaeton_codec_1.codec.decode(schema_1.validatorsSchema, validatorsBuffer);
        ...
        const encodedValidators = phaeton_codec_1.codec.encode(schema_1.validatorsSchema, { validators: nextValidatorSet });
