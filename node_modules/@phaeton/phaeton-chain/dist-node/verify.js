...
const debug = createDebug('phaeton:chain:verify');
...
const verifyBlockGenerator = async (header, slots, stateStore) => {
    ...
    const generatorAddress = phaeton_cryptography_1.getAddressFromPublicKey(header.generatorPublicKey);
...
const isValidSeedReveal = (blockHeader, stateStore, numberOfValidators) => {
    ...
    const newBlockSeedRevealBuffer = phaeton_cryptography_1.hash(newBlockSeedReveal).slice(0, SEED_REVEAL_BYTE_SIZE);
