...
class Transaction {
    ...
    static decode(bytes) {
        const tx = phaeton_codec_1.codec.decode(exports.transactionSchema, bytes);
        return new Transaction(tx);
    }
    get id() {
        if (!this._id) {
            this._id = phaeton_cryptography_1.hash(this.getBytes());
        }
        return this._id;
    }
    get senderAddress() {
        if (!this._senderAddress) {
            this._senderAddress = phaeton_cryptography_1.getAddressFromPublicKey(this.senderPublicKey);
        }
        return this._senderAddress;
    }
    getBytes() {
        const transactionBytes = phaeton_codec_1.codec.encode(exports.transactionSchema, this);
        return transactionBytes;
    }
    getSigningBytes() {
        const transactionBytes = phaeton_codec_1.codec.encode(exports.transactionSchema, {
            ...this,
            signatures: [],
        });
        return transactionBytes;
    ...
    validate(input) {
        const schemaErrors = phaeton_validator_1.validator.validate(exports.transactionSchema, this);
        if (schemaErrors.length > 0) {
            throw new phaeton_validator_1.PhaetonValidationError(schemaErrors);
        }
