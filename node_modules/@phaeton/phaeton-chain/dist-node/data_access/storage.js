...
class Storage {
    ...
    async getBlockHeadersByIDs(arrayOfBlockIds) {
        ...
        for (const id of arrayOfBlockIds) {
            try {
                ...
                if (dbError instanceof phaeton_db_1.NotFoundError) {
                    continue;
                }
                throw dbError;
            }
    ...
    async getBlockHeaderByHeight(height) {
        const stringHeight = phaeton_db_1.formatInt(height);
    ...
    async getBlockHeadersByHeightBetween(fromHeight, toHeight) {
        const stream = this._db.createReadStream({
            gte: `${constants_1.DB_KEY_BLOCKS_HEIGHT}:${phaeton_db_1.formatInt(fromHeight)}`,
            lte: `${constants_1.DB_KEY_BLOCKS_HEIGHT}:${phaeton_db_1.formatInt(toHeight)}`,
            reverse: true,
        });
    ...
    async getLastBlockHeader() {
        const stream = this._db.createReadStream({
            gte: phaeton_db_1.getFirstPrefix(constants_1.DB_KEY_BLOCKS_HEIGHT),
            lte: phaeton_db_1.getLastPrefix(constants_1.DB_KEY_BLOCKS_HEIGHT),
            reverse: true,
            limit: 1,
        });
        ...
        if (!blockID) {
            throw new phaeton_db_1.NotFoundError('Last block header not found');
        }
    ...
    async getTempBlocks() {
        const stream = this._db.createReadStream({
            gte: phaeton_db_1.getFirstPrefix(constants_1.DB_KEY_TEMPBLOCKS_HEIGHT),
            lte: phaeton_db_1.getLastPrefix(constants_1.DB_KEY_TEMPBLOCKS_HEIGHT),
            reverse: true,
        });
    ...
    async getAccountsByPublicKey(arrayOfPublicKeys) {
        const addresses = arrayOfPublicKeys.map(phaeton_cryptography_1.getAddressFromPublicKey);
    ...
    async saveBlock(id, height, finalizedHeight, header, payload, stateStore, removeFromTemp = false) {
        const heightStr = phaeton_db_1.formatInt(height);
    ...
    async deleteBlock(id, height, txIDs, fullBlock, stateStore, saveToTemp = false) {
        ...
        const { created: createdStates, updated: updatedStates, deleted: deletedStates, } = phaeton_codec_1.codec.decode(schema_1.stateDiffSchema, stateDiff);
    ...
    async _cleanUntil(height) {
        await this._db.clear({
            gte: `${constants_1.DB_KEY_DIFF_STATE}:${phaeton_db_1.formatInt(0)}`,
            lt: `${constants_1.DB_KEY_DIFF_STATE}:${phaeton_db_1.formatInt(height)}`,
        });
    }
