    ...
    constructor({ accounts, chain, consensus, defaultAccount, lastBlockHeaders, lastBlockReward, networkIdentifier, } = {}) {
        ...
        this.account = {
            get: async (address) => {
                ...
                return phaeton_utils_1.objects.cloneDeep(account);
            },
            getOrDefault: async (address) => {
                const account = this.accountData.find(acc => acc.address.equals(address));
                if (!account) {
                    return phaeton_utils_1.objects.cloneDeep({ ...this._defaultAccount, address });
                }
                return phaeton_utils_1.objects.cloneDeep(account);
            },
        ...
        this.chain = {
            ...
            get: async (key) => Promise.resolve(phaeton_utils_1.objects.cloneDeep(this.chainData[key])),
            set: async (key, value) => {
                this.chainData[key] = value;
            },
        };
        this.consensus = {
            get: async (key) => Promise.resolve(phaeton_utils_1.objects.cloneDeep(this.consensusData[key])),
            set: async (key, value) => {
