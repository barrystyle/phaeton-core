...
class P2P extends events_1.EventEmitter {
    constructor(config) {
        ...
        this._rpcSchemas = config.customNodeInfoSchema
            ...
            : schema_1.defaultRPCSchemas;
        phaeton_codec_1.codec.addSchema(this._rpcSchemas.peerInfo);
        phaeton_codec_1.codec.addSchema(this._rpcSchemas.nodeInfo);
        phaeton_codec_1.codec.addSchema(this._rpcSchemas.peerRequestResponse);
        this._networkStats = {
            startTime: Date.now(),
            incoming: {
                count: 0,
        ...
        this._handlePeerPoolMessage = (message) => {
            ...
            if (message.event === events_2.REMOTE_EVENT_POST_NODE_INFO) {
                const decodedNodeInfo = phaeton_codec_1.codec.decode(schema_1.nodeInfoSchema, message.data);
        ...
        this._handleInboundPeerConnect = (incomingPeerConnection) => {
            if (!phaeton_validator_1.isIPV4(incomingPeerConnection.socket.remoteAddress)) {
        ...
        this._nodeInfo = {
            ...config.nodeInfo,
            nonce: phaeton_cryptography_1.getRandomBytes(constants_1.DEFAULT_NONCE_LENGTH_BYTES).toString('hex'),
        };
    ...
    _handleGetPeersRequest(request) {
        ...
        const encodedResponse = phaeton_codec_1.codec.encode(this._rpcSchemas.peerRequestResponse, {
