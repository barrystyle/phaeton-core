...
const validatePeerAddress = (ipAddress, port) => {
    if (!phaeton_validator_1.isIPV4(ipAddress) || !phaeton_validator_1.isPort(port.toString())) {
        return false;
    }
    return true;
};
...
const validateRPCRequest = (request) => {
    const errors = phaeton_validator_1.validator.validate(schemas_1.rpcRequestSchema, request);
    if (errors.length) {
...
const validateProtocolMessage = (message) => {
    const errors = phaeton_validator_1.validator.validate(schemas_1.protocolMessageSchema, message);
    if (errors.length) {
...
const validatePacket = (packet) => {
    const errors = phaeton_validator_1.validator.validate(schemas_1.packetSchema, packet);
    if (errors.length) {
