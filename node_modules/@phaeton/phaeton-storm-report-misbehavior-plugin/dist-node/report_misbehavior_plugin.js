...
const phaeton_chain_1 = require("@phaeton/phaeton-chain");
const phaeton_cryptography_1 = require("@phaeton/phaeton-cryptography");
const phaeton_storm_1 = require("phaeton-storm");
const phaeton_utils_1 = require("@phaeton/phaeton-utils");
const db_1 = require("./db");
const config = require("./defaults");
const schema_1 = require("./schema");
const packageJSON = require('../package.json');
const actionParamsSchema = {
    $id: 'phaeton/report_misbehavior/auth',
    type: 'object',
...
class ReportMisbehaviorPlugin extends phaeton_storm_1.BasePlugin {
    constructor() {
        super(...arguments);
    ...
    get actions() {
        return {
            authorize: (params) => {
                const errors = phaeton_validator_1.validator.validate(actionParamsSchema, params);
                if (errors.length) {
                    throw new phaeton_validator_1.PhaetonValidationError([...errors]);
                }
                ...
                try {
                    const parsedEncryptedPassphrase = phaeton_cryptography_1.parseEncryptedPassphrase(this._options.encryptedPassphrase);
                    const passphrase = phaeton_cryptography_1.decryptPassphraseWithPassword(parsedEncryptedPassphrase, password);
                    const { publicKey } = phaeton_cryptography_1.getAddressAndPublicKeyFromPassphrase(passphrase);
    ...
    async load(channel) {
        ...
        this._options = phaeton_utils_1.objects.mergeDeep({}, config.defaultConfig.default, this.options);
    ...
    _subscribeToChannel() {
        this._channel.subscribe('app:network:event', async (eventData) => {
            ...
            if (event === 'postBlock') {
                const errors = phaeton_validator_1.validator.validate(schema_1.postBlockEventSchema, data);
                if (errors.length > 0) {
                    this._logger.error(errors, 'Invalid block data');
                    return;
                }
                const blockData = data;
                const { header } = phaeton_codec_1.codec.decode(this.schemas.block, Buffer.from(blockData.block, 'hex'));
                try {
    ...
    async _createPoMTransaction(contradictingBlock, decodedBlockHeader) {
        ...
        const encodedAccount = await this._channel.invoke('app:getAccount', {
            address: phaeton_cryptography_1.getAddressFromPassphrase(passphrase).toString('hex'),
        });
        const { sequence: { nonce }, } = phaeton_codec_1.codec.decode(this.schemas.account, Buffer.from(encodedAccount, 'hex'));
        ...
        const encodedAsset = phaeton_codec_1.codec.encode(pomAssetInfo.schema, pomTransactionAsset);
        const tx = new phaeton_chain_1.Transaction({
            moduleID: pomAssetInfo.moduleID,
            assetID: pomAssetInfo.assetID,
            nonce,
            senderPublicKey: (_a = this._state.publicKey) !== null && _a !== void 0 ? _a : phaeton_cryptography_1.getAddressAndPublicKeyFromPassphrase(passphrase).publicKey,
            fee: BigInt(this._options.fee),
            asset: encodedAsset,
            signatures: [],
        });
        tx.signatures.push(phaeton_cryptography_1.signData(Buffer.concat([Buffer.from(networkIdentifier, 'hex'), tx.getSigningBytes()]), passphrase));
